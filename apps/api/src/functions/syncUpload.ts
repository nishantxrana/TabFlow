 ...
 */
function validateRequestBody(body: SyncUploadRequestBody): { error: string; code: string } | null {
  ...
  if (body.clientTimestamp === undefined || typeof body.clientTimestamp !== "number") {
    return {
  ...
  if (body.clientTimestamp < now - oneYearMs || body.clientTimestamp > now + oneYearMs) {
    return {
...
async function syncUpload(
  ...
  try {
    ...
    if (DEV_MODE_ENABLED && devUserId) {
      context.warn("[syncUpload] DEV_MODE: Using mock user ID - MUST BE DISABLED IN PRODUCTION");
    ...
    try {
      ...
      return jsonResponse({ error: "Invalid JSON body", code: "INVALID_JSON" }, 400, origin);
    }
    ...
    if (!uploadResult.success) {
      context.error(`[syncUpload] STORAGE_ERROR: ${uploadResult.code} user=${userIdShort}`);
      return jsonResponse({ error: uploadResult.error, code: uploadResult.code }, 500, origin);
    }
    ...
    context.log(`[syncUpload] SUCCESS: user=${userIdShort} synced=${uploadResult.syncedAt}`);
